...
def run_swarp(input_args, img_dir, wht_dir, outdir, keyw_dict, swarp_config,
    ...
    # Based on the field, define the field centre to be input to swarp
    if field == "EN1":
        ra_cent = 242.74788  # CORRECT VALUES
        dec_cent = 55.01753
        # dec_cent = 54.75061855
    elif field == "Lockman":
        ra_cent = 161.9498
        dec_cent = 57.9884
    else:
        print("Field centre not defined!")
    ...
    if do_flxscale[0].lower() == "y":
        ...
        for img in img_list:
            img_tmp = img.replace("[1]", "")
            with fits.open(img_tmp, mode='update') as hdul:
                hdr = hdul[0].header
